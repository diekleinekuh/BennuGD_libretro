...
static int        iterator_pos          = INSTANCE_MAX_PRIORITY;
...
INSTANCE * instance_get_by_type( uint32_t type, INSTANCE ** context )
{
    ...
    else if ( ( i = *context ) == ( INSTANCE * ) ptr_from_int(0xFFFFFFFF) ) /* End scan */
        return ( *context = NULL );
    ...
    if ( i ) /* Valid instance, continue scan */
    {
        if ( i->next_by_type )
            ...
            *context = ( INSTANCE * ) ptr_from_int(0xFFFFFFFF) ; /* Next call will be "end scan" */
